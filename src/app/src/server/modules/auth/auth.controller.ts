...
import ValidateCustomerPasswordOnSignIn
	from 'Server/modules/auth/pipes/Customer/OnSignIn/ValidateCustomerPassword.pipe';
...
export class AuthController {
  ...
  async signInAdmin(
	...
	const { access_token } = await this.authService.signInAdmin(login, password);
  ...
  async signIn(
	...
	const { access_token } = await this.authService.signInCustomer(login, password);
	session['access_token'] = access_token;
  ...
  async signUp(
	...
	const { access_token } = await this.authService.signUpCustomer(login, password);
	session['access_token'] = access_token;
